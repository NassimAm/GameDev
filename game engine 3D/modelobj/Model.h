...
public:
    ...
    Model(Loader loader,string const &path, bool gamma = false) : gammaCorrection(gamma)
    {
        this->loader = loader;
        loadModel(path);
    }
...
private:
    ...
    glm::vec3 axerotation = glm::vec3(1.0f,0.0f,0.0f);
    float rotation = 0.0f;
    ...
    vector<Texture> loadMaterialTextures(aiMaterial *mat, aiTextureType type, string typeName)
    {
        ...
        for(unsigned int i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            {   // if texture hasn't been loaded already, load it
                Texture texture;
                texture.id = loader.TextureFromFile(str.C_Str(), this->directory);
